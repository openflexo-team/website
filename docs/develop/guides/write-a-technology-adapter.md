---
sidebar_position: 1
title: Write a technology adapter
---

# Write a technology adapter
...
Every technology adapter follows the same fixed multi-module template — the `xx-ta` skeleton — and
a new one is scaffolded and wired end to end with the `add-technology-adapter` guided procedure
rather than by hand. A worked walkthrough (basic structure, examples, testing, common pitfalls)
covering one TA built from scratch is not written yet.