...
* `docs/develop/components.md` (position 7), generated by the new `scripts/generate_components_page.py`
  from `src/data/components.json` and `src/data/technology-adapters.json`: generic frameworks, core
  components, and the technology adapters grouped by status, as plain link lists and tables (no
  logo cards — simplified on purpose).
* `docs/community/partners.md` (position 2) and `docs/research/people.md` (position 7): hand-written,
  since their source lists are small and rarely change. See WEB-F-13 for `partners.md`.
* The three old page files, `src/components/Partners` and `src/components/Contributors` (their only
  callers) are deleted. `sidebars.js` no longer needs the `{type: 'link', …}` entries: the three
  pages now appear in their sidebar through the ordinary `autogenerated` mechanism.
